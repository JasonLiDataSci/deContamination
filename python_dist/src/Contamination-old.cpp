...
void order_descending_index(double *arr, int n, int *idx) {
    if (arr == nullptr || idx == nullptr || n <= 0) return;
    ...
    // sort indices by corresponding arr values (descending)
    std::sort(idx, idx + n, [arr](int i, int j) {
        return arr[i] > arr[j];
    });
}
...
#pragma omp parallel for
    ...
    auto *mu_norm = new double[K]();
    int *mu_index = new int[K]();
    auto **mu_t_reorder = new double *[G];
    for (int g = 0; g < G; ++g) {
        mu_t_reorder[g] = new double[K]();
    }
    auto *pi_t_reorder = new double[K]();
    for (int k = 0; k < K; ++k) {
        pi_t_reorder[k] = 1.0 / K;
    }
...
#pragma omp parallel for
        for (int k = 0; k < K; ++k) {
            ...
            for (int g = 0; g < G; ++g) {
                mu_norm[k] += pow(mu_t[g][k], 2);
            }
        }

        order_descending_index(mu_norm, K, mu_index);

#pragma omp parallel for
        for (int g = 0; g < G; ++g) {
            for (int k = 0; k < K; ++k) {
                mu_t_reorder[g][k] = mu_t[g][mu_index[k]];
            }
        }
...
#pragma omp parallel for
        for (int k = 0; k < K; ++k) {
            pi_t_reorder[k] = pi_t[mu_index[k]];
        }
#pragma omp parallel for
        for (int k = 0; k < K; ++k) {
            pi_t[k] = pi_t_reorder[k];
        }
